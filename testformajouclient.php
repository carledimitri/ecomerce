...
if (empty($errors)) {
  ...
  exit();
    ...
    //TEST ADRESS EMAIL
    // $req=$conn->prepare("SELECT COUNT(*) AS NbreEmail from clients WHERE EmailCli = ? ");
    // $req->execute(array($EmailCli));

    // while ($email_verification=$req->fetch()) {

    //   if ($email_verification['NbreEmail'] != 0 ){
    //     header('location: formajouclient.php?error=1&email=1');
    //     return ;
    //   }
    ...
    // //TEST Nom
    // $req=$conn->prepare("SELECT COUNT(*) AS NbreNom from clients WHERE NomCli = ? ");
    // $req->execute(array($NomCli));

    // while ($nom_verification=$req->fetch()) {

    //   if ($nom_verification['NbreNom'] != 0 ){
    //     header('location: formajouclient.php?error=1&nom=1');
    //     return ;
    //   }
    ...
    // header('location: formajouclient.php?success=1');
...
?>
  ...
  <h1 style="color: #ff9f43;" class=" display-1 ">Ajout Client </h1>   
   ...
   <?php  if (!empty($errors)): ?>
    ...
    <form method="POST" style="color: #ff9f43;" action="testformajouclient.php"  >
